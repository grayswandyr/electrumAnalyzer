...
fact Traces {
	...
	all t: Time-to/last |
		let t8 = t.to/next |
			some e: Event {
				e.pre = t and e.post = t8
				currentKey.t != currentKey.t8 => e in Entry
				occupant.t != occupant.t8 => e in Checkin + Checkout
				(lastKey.t != lastKey.t8 or keys.t != keys.t8) => e in Checkin
			}
	}
