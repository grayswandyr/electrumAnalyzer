...
pred allExist {
  ...
  (all s: Seq | SeqIdx !in s.inds => (all e: elem | some s8: Seq | s.add[e, s8]))
}
...
pred allExistNoDuplicates {
  ...
  all s: Seq {
    !s.hasDups
    SeqIdx !in s.inds => (all e: elem - s.elems | some s8: Seq | s.add[e, s8])
  }
}
...
/* returns first index at which given element appears or the empty set if it doesn8t */
fun idxOf [s: Seq, e: elem] : lone SeqIdx { ord/min[s.indsOf[e]] }

/* returns last index at which given element appears or the empty set if it doesn8t */
fun lastIdxOf [s: Seq, e: elem] : lone SeqIdx { ord/max[s.indsOf[e]] }

/* returns set of indices at which given element appears or the empty set if it doesn8t */
