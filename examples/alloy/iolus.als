...
abstract sig KDS {
  ...
  Monotonic[keys]
  all t : Tick | let t8 = ord/prev[t] {
    all m : members[t]-members[t8] | Join[m, t, this]
    all m : members[t8]-members[t] | Leave[m, t]
  }
}
...
{ SendMessage[sender, sentTime, this] ||
  (some msg8 : DataMessage |
     Remulticast[gsaID, msg8, retransmitTime, this]) }
...
fact IolusProperties {
  no k, k8 : GroupKey | k!=k8 && k.generator = k8.generator && k.generatedTime = k8.generatedTime
  all g : GSA, msg : DataMessage, t : Tick | RemulticastConditions[g, msg, t] =>
    (some msg8: DataMessage | Remulticast[g, msg, t, msg8])
}
...
fact {
  ...
  no m, m8 : DataMessage {
    m!=m8
    m.sender = m8.sender
    m.sentTime = m8.sentTime
    m.key = m8.key
    m.gsaID = m8.gsaID
    m.retransmitTime = m8.retransmitTime }
}
...
pred SendRequest[c : Client, gsa : GSA, t : Tick, msg : DataMessage] {
  ...
  (some gsa.parent.members[t]) =>
    (some msg8 : DataMessage | Remulticast[gsa, msg, t, msg8]) }
...
pred CanReceive[m : Member, t : Tick, msg : Message] {
  some msg8 : DataMessage {
    msg8.sentTime = msg.sentTime
    msg8.sender = msg.sender
    msg8 in m.receivedMessages[ord/prev[t]] || ReceiveConditions[m, t, msg8] }}
...
pred Remulticast[g : GSA, msg : DataMessage, t : Tick, msg8: lone DataMessage] {
  RemulticastConditions[g, msg, t]
  let g8 = g.parent + g - msg.gsaID | NewestKey[g8.keys[msg.sentTime], msg8.key]
  msg8.sender = msg.sender
  msg8.sentTime = msg.sentTime
  msg8.retransmitTime = t
  msg8.gsaID = g
}
...
assert Connected {
  all g, g8 : GSA | g in g8.*(parent + ~parent) }
...
pred LoopFree {
  no t, t8 : Tick {
    t!=t8
    all k : KDS | k.members[t] = k.members[t8] -- no constraint on keys
    all m : Member | m.receivedMessages[t] = m.receivedMessages[t8]
    all m : DataMessage | m.retransmitTime = t =>
      (some m8 : DataMessage {
        m8.retransmitTime = t8
        m.sender = m8.sender
        m.sentTime = m8.sentTime
        m.gsaID = m8.gsaID
        m.key = m8.key })
...
assert NonLinearTopology {
  (no g : GSA | #g.~parent > 1) ||
  !(some m, m8 : DataMessage | Remulticast[m.gsaID, m8, m.retransmitTime, m]
...
assert x {
  !(LoopFree && some DataMessage &&
     (some t : Tick | some m, m8 : Member |
        m!=m8 && IsMember[m, t] && IsMember[m8, t] && t != ord/next[ord/first]))
}
...
assert InsiderCanRead {
  all msg : Message, m : Member |
    some t : Tick - ord/last | all t8 : ord/nexts[t] |
      (IsMember[msg.sender, msg.sentTime] &&
       IsMember[m, msg.sentTime]) => CanReceive[m, t8, msg]
}
