 ...
 * 8 hands. So answers must be 0..8. The one (p8 say) who answered 8 has shaken everybody8s
 * hand except for his or her own, and his or her spouse8s. Now consider the person who shook
 ...
 * So p88s spouse to p0. Now imagine Jocelyn asking the question again, with p0 and p8 out of
...
fact ShakingProtocol {
    // nobody shakes own or spouse8s hand
...
fact Spouses {
    all p, q: Person | p!=q => {
        // if q is p8s spouse, p is q8s spouse
        p.spouse = q => q.spouse = p
        // no spouse sharing
        p.spouse != q.spouse
        }
    all p: Person {
        // a person is his or her spouse8s spouse
        p.spouse.spouse = p
