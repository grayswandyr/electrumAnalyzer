...
pred TRActPreConds[p : Process, pre : State] {
  // can8t already have a level
  no pre.lvl[p]
  // must have a neighbor with a set level so
  // p can read it
  // Root doesn8t need to read value from a
  // neighbor
  ...
  (p = Root) => {
    ...
    some adjProc: p.adj |
      let nLvl = pre.lvl[adjProc] | {
        some nLvl
        // p8s parent is the adjacent
        // process, and p8s level is one greater than
...
fact LegalTrans {
  ...
  all s : State - so/last |
    let s8 = so/next[s] | {
      all p : Process |
        p in s.runs => Trans[p, s, s8] else TRNop[p,s,s8]
    }
}

pred PossTrans[s, s8 : State] {
  all p : Process | Trans[p,s,s8]
}
...
pred TraceWithoutLoop {
  all s, s8 : State | s!=s8 => {
    !EquivStates[s, s8]
    (s8 in so/nexts[s] && (s8 != so/next[s])) => !PossTrans[s,s8]
  }
...
pred BadLivenessTrace {
  ...
  some s, s8 : State | s!=s8 && EquivStates[s, s8]
  all s : State | !SpanTreeAtState[s]
}
