 ...
 * than the process8 own ID it forwards the ID to its right
...
pred ValidTrans2[s, s8: State] {
  all p : s.runs | VT2Helper[p,s,s8]
  all p : Process - s.runs | NOP2[p,s,s8]
  NoMagicMsg[s,s8]

}

pred NoMagicMsg[s, s8 : State] {
    // no magically appearing messages
    all p : Process, m : s8.buffer[p] |
      m in s.buffer[p] || (!NOP2[p,s,s8] &&
                            ((s = so/first && m = p) ||
                             (s != so/first && m in s.buffer[p.~rightNeighbor]
                              && m !in s8.buffer[p.~rightNeighbor] && po/gt[m,p])))
}

pred PossTrans[s, s8 : State] {
  all p : Process | VT2Helper[p,s,s8] || NOP2[p,s,s8]
  NoMagicMsg[s,s8]
}

pred VT2Helper[p : Process, s, s8 : State] {
    (
      let readable=s.buffer[p.~rightNeighbor] |
        (s = so/first) => {
          p = s8.buffer[p]
          readable in s8.buffer[p.~rightNeighbor]
          p !in s8.leader
        } else {
          (some readable) => {
           some m : set readable | {
             m !in s8.buffer[p.~rightNeighbor]
             // nothing else gets deleted
             readable - m in s8.buffer[p.~rightNeighbor]
             { m8: m | po/gt[m8,p] } /*m & nexts(p)*/ in s8.buffer[p]
             p in s8.leader iff (p in s.leader || p in m)
           }
          } else NOP2[p,s,s8]
        }
    )
...
pred NOP2[p : Process, s,s8: State] {
  p in s8.leader iff p in s.leader
  // no reads
  s.buffer[p.~rightNeighbor] in s8.buffer[p.~rightNeighbor]
  // no sends
  s8.buffer[p] in s.buffer[p]
}
...
pred TraceWithoutLoop  {
  ...
  all s, s8 : State | (s8 in (so/nexts[s] - so/next[s])) => !PossTrans[s,s8]
  all s : State | !Legit[s]
}
