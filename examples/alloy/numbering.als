 ...
 * become children of appendix. We8ll call the set of styles active in the tree at a given time
 * the "context".
 *
 ...
 * (a document can8t start with subsection, eg). The assertion PreservesForest checks that the
 ...
 * not decreased. The first assertion is valid; the second isn8t.
 *
...
pred AddStyleToContext [s, s8: State, style: Style] {
    all x: style.^parents | some x.replaces & s.context
    s8.context = s.context - style.replaces + style
    }

assert PreserveForest {
    all s,s8: State, z: Style |
        Forest[s] && AddStyleToContext [s,s8,z] => Forest[s8]
    }
...
pred AddStyleToNumbering [s, s8: State, style: Style] {
    s8.value[style] = (style in s.context => s.value[style].next else style.initial)
    s8.context = s.context - style.replaces + style
    all x: Style - style |
        s8.value[x] = (style in x.^parents => x.initial else s.value[x])
    }

pred AddStyle [s, s8: State, style: Style] {
    AddStyleToContext [s,s8,style]
    AddStyleToNumbering [s,s8,style]
    }

assert AddNeverReduces {
    all s,s8: State, z: Style |
        Forest[s] && AddStyle [s,s8,z] =>
            (all y: s8.context | s8.value[y] in s.value[y].*next)
    }
