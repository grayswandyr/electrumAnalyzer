...
pred TrainsMove [x, x8: TrainState, ts: set Train] {
  all t: ts | t.(x8.on) in t.(x.on).next
  all t: Train - ts | t.(x8.on) = t.(x.on)
  }
...
assert PolicyWorks {
  all x, x8: TrainState, g: GateState, ts: set Train |
    {MayMove [g, x, ts]
    TrainsMove [x, x8, ts]
    Safe [x]
    GatePolicy [g, x]
...
pred TrainsMoveLegal [x, x8: TrainState, g: GateState, ts: set Train] {
  TrainsMove [x, x8, ts]
  MayMove [g, x, ts]
  GatePolicy [g, x]
  }
