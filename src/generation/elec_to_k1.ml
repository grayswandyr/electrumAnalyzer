...
and pred_to_form (pred:Ast_par.pred) args envalias_init=
  let envalias_glob={envalias_init with curterm=None; local_names=[]} in (*to avoid adding some 'one'*)
  let envalias_loc={envalias_init with curterm=None} in (* environment with local names, for the arguments *)
  let open Ast_par in
  ...
  let (argterms, flist)=List.fold_left (
        fun (accarg,accf) arg->
          let (targ, f)=expr_to_k1term arg envalias_loc
          in targ::accarg, f@accf
      ) 
        ([],[]) args in
  let pbody=List.map (fun x -> expr_to_k1prop x envalias_glob) pred.body in
...
and function_to_term (fct : Ast_par.func) args envalias_init=
  let envalias_glob={envalias_init with curterm=None; local_names=[]} in (*to avoid adding some 'one', and not dereference the body*)
  let envalias_loc={envalias_init with curterm=None} in (* environment with local names, for the arguments *)
  let open Ast_par in
  let open Ast_expr in
  (* we reverse the order of the two lists to win some time *)
  ...
  (*let (argterms, flist) =
    ...
    args 
    in*)
  let t_body, f_body  = expr_to_k1term  fct.body envalias_glob in
  ...
  *)
  ...
  in join_appl res_fct rest envalias_loc
